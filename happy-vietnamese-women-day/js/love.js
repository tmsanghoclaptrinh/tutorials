(function(window) {
    ...
    Tree.prototype = {
        ...
        initBloom: function() {
            ...
            for (var i = 0; i < 100; i++) {
                if (i % 2) {
                    cache.push(this.createBloomImage(width, height, r, figure));
                } else {
                    cache.push(this.createBloom(width, height, r, figure));
                }
            }
        ...
        createBloomImage: function(width, height, radius, figure, color, alpha, angle, scale, place, speed) {
            var x, y;
            while (true) {
                x = random(20, width - 20);
                y = random(20, height - 20);
                if (inheart(x - width / 2, height - (height - 40) / 2 - y, radius)) {
                    return new BloomImage(this, new Point(x, y), figure, color, alpha, angle, scale, place, speed);
                }
            }
        },
    ...
    Bloom = function(tree, point, figure, color, alpha, angle, scale, place, speed) {
        this.tree = tree;
        this.point = point;
        this.color = color || 'rgb(255,' + random(0, 150) + ',' + random(0, 150) + ')';
    ...
    BloomImage = function(tree, point, figure, color, alpha, angle, scale, place, speed) {
        this.tree = tree;
        this.point = point;
        this.color = 'white';
        this.alpha = alpha || random(0.3, 1);
